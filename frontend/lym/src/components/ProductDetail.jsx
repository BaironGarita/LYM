    ...
    const fetchData = async () => {
      try {
        const [productResponse, imagesResponse] = await Promise.all([
          fetch(`http://localhost:8000/?url=productos&id=${id}`),
          fetch(`http://localhost:8000/?url=productos/imagenes&producto_id=${id}`)
        ]);

        if (!productResponse.ok) {
          throw new Error(`Error al obtener el producto: ${productResponse.statusText}`);
        }
              ...
              {imagenes.length > 1 && (
                <div className="flex gap-3 overflow-x-auto pb-2">
                  {imagenes.map((img, index) => (
                    ...
                    >
                      <img
                        src={`http://localhost:81/LYM/api/${img.ruta_archivo}`}
                  ...
                  <div className="flex items-center gap-1">
                    {[...Array(5)].map((_, i) => (
                      <Star
                        key={i}
                        className={`h-5 w-5 ${
                          i < Math.floor(getRatingValue(product.promedio_valoracion))
