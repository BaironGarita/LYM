class PromocionService {
  static async getPromociones() {
    try {
      const response = await fetch('http://localhost:8000/?url=promociones');
      if (!response.ok) {
  ...
  static calcularPrecioConDescuento(producto, promociones) {
    ...
    console.log('Calculando precio para producto:', producto.id, producto.nombre);
    ...
    const promocionesValidas = promociones.filter(promo => {
      try {
        ...
        const esActiva = promo.activa == 1 || promo.activo == 1 || promo.activa === true || promo.activo === true;
        const estaEnFechas = ahora >= fechaInicio && ahora <= fechaFin;
        
        console.log(`Promoción ${promo.id}: activa=${esActiva}, fechas=${estaEnFechas}`, {
    ...
    promocionesValidas.forEach(promo => {
      ...
      if (promo.tipo === 'producto' && String(promo.producto_id) === String(producto.id)) {
        aplicable = true;
        console.log(`Promoción por producto aplicable: ${promo.nombre}`);
      }
      
      // Promoción por categoría - comparar como strings
      if (promo.tipo === 'categoria' && String(promo.categoria_id) === String(producto.categoria_id)) {
        aplicable = true;
      ...
      if (aplicable && parseFloat(promo.porcentaje) > mejorDescuento) {
        ...
        console.log(`Nueva mejor promoción: ${promo.nombre} - ${mejorDescuento}%`);
      }
    });
